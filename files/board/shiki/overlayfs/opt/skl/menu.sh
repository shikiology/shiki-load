  ...
  while true; do
    ...
    case "`<${TMP_PATH}/resp`" in
      a)
        ...
        TAG="`curl --insecure -s https://api.github.com/repos/andatoshiki/shiki/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`"
        ...
        STATUS=`curl --insecure -w "%{http_code}" -L \
          "https://github.com/andatoshiki/shiki/releases/download/${TAG}/update.zip" -o /tmp/update.zip`
        if [ $? -ne 0 -o ${STATUS} -ne 200 ]; then
          dialog --backtitle "`backtitle`" --title "Update shiki" --aspect 18 \
      ...
      d)
        ...
        STATUS=`curl --insecure -s -w "%{http_code}" -L "https://github.com/andatoshiki/shiki-addons/releases/download/${TAG}/addons.zip" -o /tmp/addons.zip`
        if [ $? -ne 0 -o ${STATUS} -ne 200 ]; then
          dialog --backtitle "`backtitle`" --title "Update addons" --aspect 18 \
      ...
      l)
        dialog --backtitle "`backtitle`" --title "Update LKMs" --aspect 18 \
          --infobox "Checking last version" 0 0
        TAG=`curl --insecure -s https://api.github.com/repos/andatoshiki/shiki-lkm/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`
        if [ $? -ne 0 -o -z "${TAG}" ]; then
          dialog --backtitle "`backtitle`" --title "Update LKMs" --aspect 18 \
            --msgbox "Error checking new version" 0 0
          continue
      ...
      m)
        ...
        dialog --backtitle "`backtitle`" --title "Update Modules" --aspect 18 \
          --infobox "Checking last version" 0 0
        TAG=`curl --insecure -s https://api.github.com/repos/andatoshiki/shiki-modules/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`
        if [ $? -ne 0 -o -z "${TAG}" ]; then
          dialog --backtitle "`backtitle`" --title "Update Modules" --aspect 18 \
            --msgbox "Error checking new version" 0 0
          continue
        ...
        for P in ${!PLATFORMS[@]}; do
          ...
          STATUS=`curl --insecure -s -w "%{http_code}" -L "https://github.com/andatoshiki/shiki-modules/releases/download/${TAG}/${P}.tgz" -o "/tmp/${P}.tgz"`
          if [ $? -ne 0 -o ${STATUS} -ne 200 ]; then
            dialog --backtitle "`backtitle`" --title "Update Modules" --aspect 18 \
