...
. /opt/shiki/include/functions.sh
. /opt/shiki/include/addons.sh
. /opt/shiki/include/modules.sh

# Check partition 3 space, if < 2GiB is necessary clean cache folder
CLEARCACHE=0
LOADER_DISK="`blkid | grep 'LABEL="SHIKI3"' | cut -d3 -f1`"
...
function updateMenu() {
  ...
  while true; do
    ...
    case "`<${TMP_PATH}/resp`" in
      a)
        dialog --backtitle "`backtitle`" --title "Update shiki" --aspect 18 \
          --infobox "Checking last version" 0 0
        ACTUALVERSION="v${SHIKI_VERSION}"
        TAG="`curl --insecure -s https://api.github.com/repos/shikiology/shiki/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`"
        ...
        STATUS=`curl --insecure -w "%{http_code}" -L \
          "https://github.com/shikiology/shiki/releases/download/${TAG}/update.zip" -o /tmp/update.zip`
        if [ $? -ne 0 -o ${STATUS} -ne 200 ]; then
          dialog --backtitle "`backtitle`" --title "Update shiki" --aspect 18 \
        ...
        dialog --backtitle "`backtitle`" --title "Update shiki" --aspect 18 \
          --yesno "SHIKI updated with success to ${TAG}!\nReboot?" 0 0
        [ $? -ne 0 ] && continue
        shiki-reboot.sh config
        exit
        ;;
      ...
      d)
        dialog --backtitle "`backtitle`" --title "Update addons" --aspect 18 \
          --infobox "Checking last version" 0 0
        TAG=`curl --insecure -s https://api.github.com/repos/shikiology/shiki-addons/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`
        if [ $? -ne 0 -o -z "${TAG}" ]; then
          dialog --backtitle "`backtitle`" --title "Update addons" --aspect 18 \
            --msgbox "Error checking new version" 0 0
          continue
        ...
        STATUS=`curl --insecure -s -w "%{http_code}" -L "https://github.com/shikiology/shiki-addons/releases/download/${TAG}/addons.zip" -o /tmp/addons.zip`
        if [ $? -ne 0 -o ${STATUS} -ne 200 ]; then
          dialog --backtitle "`backtitle`" --title "Update addons" --aspect 18 \
      ...
      l)
        dialog --backtitle "`backtitle`" --title "Update LKMs" --aspect 18 \
          --infobox "Checking last version" 0 0
        TAG=`curl --insecure -s https://api.github.com/repos/shikiology/shiki-lkm/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`
        if [ $? -ne 0 -o -z "${TAG}" ]; then
          dialog --backtitle "`backtitle`" --title "Update LKMs" --aspect 18 \
            --msgbox "Error checking new version" 0 0
          continue
        ...
        dialog --backtitle "`backtitle`" --title "Update LKMs" --aspect 18 \
          --infobox "Downloading last version" 0 0
        STATUS=`curl --insecure -s -w "%{http_code}" -L "https://github.com/shikiology/shiki-lkm/releases/download/${TAG}/shiki-lkms.zip" -o /tmp/shiki-lkms.zip`
        if [ $? -ne 0 -o ${STATUS} -ne 200 ]; then
          dialog --backtitle "`backtitle`" --title "Update LKMs" --aspect 18 \
      ...
      m)
        ...
        dialog --backtitle "`backtitle`" --title "Update Modules" --aspect 18 \
          --infobox "Checking last version" 0 0
        TAG=`curl --insecure -s https://api.github.com/repos/shikiology/shiki-modules/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`
        if [ $? -ne 0 -o -z "${TAG}" ]; then
          dialog --backtitle "`backtitle`" --title "Update Modules" --aspect 18 \
            --msgbox "Error checking new version" 0 0
          continue
        ...
        for P in ${!PLATFORMS[@]}; do
          ...
          STATUS=`curl --insecure -s -w "%{http_code}" -L "https://github.com/shikiology/shiki-modules/releases/download/${TAG}/${P}.tgz" -o "/tmp/${P}.tgz"`
          if [ $? -ne 0 -o ${STATUS} -ne 200 ]; then
            dialog --backtitle "`backtitle`" --title "Update Modules" --aspect 18 \
